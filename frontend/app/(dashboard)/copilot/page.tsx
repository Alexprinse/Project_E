...
export default function CopilotPage() {
  const { messages, loading, status, citations, confidence, sendMessage, clearChat } =
    useStreamingChat();
  ...
  return (
    <div className="flex-1 flex overflow-hidden h-full relative no-zoom">
      ...
      <div className="flex-1 flex flex-col h-full bg-background border-r border-border min-w-0">
        ...
        {/* ── Conversation Area ──
            KEY: flex-1 min-h-0 lets this shrink when the keyboard opens on iOS.
            overflow-y-auto + scroll-touch gives momentum scrolling. */}
        <div className="flex-1 min-h-0 overflow-y-auto scroll-touch p-4 md:p-6 space-y-4 md:space-y-6">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 md:p-8 max-w-sm mx-auto space-y-4">
              ...
              <div className="w-full space-y-2">
                <button
                  onClick={() => sendMessage("What is the discharge pressure target for Pump P-101?")}
                ...
                <button
                  onClick={() => sendMessage("List failure records on heat exchanger HX-302")}
                  className="w-full text-left text-[10px] font-mono p-3 border border-border bg-muted/10 rounded hover:border-slate-500 text-slate-300 flex justify-between items-center group min-h-[48px] tap-target active:bg-muted/30"
                >
